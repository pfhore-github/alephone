	...
	short flags)
{
	...
	auto s = get_picture_resource_from_scenario(picture_id, PictRsrc) ? picture_to_surface(PictRsrc) : std::shared_ptr<SDL_Surface>(nullptr, SDL_FreeSurface);
...
{
	return SIZEOF_static_preprocessed_terminal_data + t.groupings.size() * SIZEOF_terminal_groupings + t.font_changes.size() * SIZEOF_text_face_data + t.text.size();
}
