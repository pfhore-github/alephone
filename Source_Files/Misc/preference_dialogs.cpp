...
//		billboard_placer->dual_add(billboard_w->label("Tilt Sprites with Camera"), m_dialog);
		billboard_placer->dual_add(billboard_w->label("カメラでスプライトを傾ける"), m_dialog);
		...
		general_table->dual_add(ephemera_w->label("スクリプト効果品質"), m_dialog);
		...
		near_filter_labels[OGL_Txtr_WeaponsInHand] = new w_label("手中の武器");
		near_filter_labels[OGL_Txtr_HUD] = new w_label("HUD / ターミナル");
		...
		texture_labels[OGL_Txtr_WeaponsInHand] = new w_label("手中の武器");
		texture_labels[OGL_Txtr_HUD] = new w_label("HUD / ターミナル");
